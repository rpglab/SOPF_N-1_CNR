...
#var cont_line_St {BRANCH, SCENARIO, CONTINGENCY};     # Variable for all line flows
#var cont_line_flow {BRANCH, SCENARIO, CONTINGENCY};     # Variable for all line flows
#var cont_bus_angle {BUS, SCENARIO, CONTINGENCY};     # Variable for all line flows
...
subject to Line_FlowEq_c{k in BRANCH, s in SCENARIO, c in CONTINGENCY: k != c}:	#Line Flow Constraint for steady-state:
	(bus_angle_c[branch_fbus[k],s,c] - bus_angle_c[branch_tbus[k],s,c]) / branch_x[k] = line_flow_c[k,s,c];
...
subject to Thermal_c1{k in BRANCH, s in SCENARIO, c in CONTINGENCY}:		# Thermal Constraint, contingency-state
	branch_rateC[k] >= line_flow_c[k,s,c]; # based on Rate C
